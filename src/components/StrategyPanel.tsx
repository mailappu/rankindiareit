...
const PRESET_INFO: Record<Exclude<StrategyPreset, 'custom'>, { label: string; desc: string }> = {
  income: { label: 'INCOME FOCUS', desc: '50% Yield · 30% Safety · 20% Value' },
  growth: { label: 'GROWTH FOCUS', desc: '40% Growth · 40% Pipeline · 20% Value' },
...
const WEIGHT_LABELS: { key: keyof StrategyWeights; label: string; color: string }[] = [
  { key: 'yield', label: 'Dividend Yield', color: 'text-terminal-green' },
  { key: 'safety', label: 'Safety', color: 'text-terminal-blue' },
  { key: 'value', label: 'Value', color: 'text-terminal-amber' },
  { key: 'growth', label: 'Price Growth', color: 'text-terminal-cyan' },
      ...
      <div className="flex gap-2 mb-4">
        {(Object.keys(PRESET_INFO) as Exclude<StrategyPreset, 'custom'>[]).map(p => (
          <button
            ...
            className={`flex-1 px-3 py-2 rounded text-xs font-mono transition-all border ${
              preset === p
          ...
          >
            <div className="font-semibold">{PRESET_INFO[p].label}</div>
            <div className="text-[10px] mt-0.5 opacity-70">{PRESET_INFO[p].desc}</div>
          </button>
        ))}
      </div>
